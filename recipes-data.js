...
// 5. Base de données unitaire des Prix d'Achats & Moteur de Calcul Dynamique du Food Cost
const INGREDIENT_UNIT_COSTS = {
  ...
  "gambas": { cost: 0.180, unit: "g" },
  "gambas panees": { cost: 0.180, unit: "g" },
  "calamar": { cost: 0.095, unit: "g" },
...
function normalizeTextSimple(str) {
  return (str || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]/g, ' ').replace(/\s+/g, ' ').trim();
}

/**
 * Calcule le coût matière, le Food Cost (%) et la Marge Brute à partir d'une liste de lignes d'ingrédients.
 * @param {Array<string>} ingredients - Ex: ["Poulet : 150 g", "Sauce : 30 g"]
 * @param {number} sellPrice - Ex: 68
 * @returns {Object} { cost, sellPrice, foodCost, margin, grossMarginDH, breakdown }
 */
function calculateRecipeFoodCost(ingredients, sellPrice) {
  ...
  (ingredients || []).forEach(line => {
    ...
    const normIng = normalizeTextSimple(ingName);
    let ingDef = INGREDIENT_UNIT_COSTS[normIng];

    if (!ingDef) {
      for (const [k, v] of Object.entries(INGREDIENT_UNIT_COSTS)) {
    ...
    if (!ingDef) {
      ingDef = { cost: 0.02, unit: "g" }; // Fallback 20 DH/kg
    }
